...
@Directive({
    selector: 'input[decimalsOnly1]'
})
export class DecimalDirective1 {
    ...
    onkeypress(e: KeyboardEvent) {
        ...
        if ((beforeDecimalLength >= 3) && (e.keyCode != 46) && (decimalIndex == -1)) {
            event.preventDefault();
        }
        else if ((beforeDecimalLength >= 3) && (decimalIndex != -1) && afterDecimalLength >= 2) {
            event.preventDefault();
        }
        else if ((beforeDecimalLength > 3) && (decimalIndex == -1) && afterDecimalLength >= 2) {
            event.preventDefault();
    ...
    @HostListener('input', ['$event']) onInputChange(event) {
        ...
        if(beforeDecimalLength>3 || afterDecimalLength > 2 || decimalPOintCount > 1){
