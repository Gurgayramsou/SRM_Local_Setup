...
export class VendorComponent implements OnInit {
    ...
    constructor(private dataService: DataService, private pagerService: PagerService, private scoreDataService: ScoreService,
        private modalService: NgbModal, private calendar: NgbCalendar, private datepipe: DatePipe, private vendorDataService: VendorService
        , private alertMailService: AlertMailService, private route: ActivatedRoute) {
    ...
    ngOnInit() {
        ...
        let getVendorDetailsByUser = this.dataService.getVendorDetailsByUser(this.loggedEnterpriseId);
        let getPackageFacilityDetailsByPage = this.dataService.getPackageFacilityDetailsByPage(this.loggedEnterpriseId, pagePath)
        forkJoin([getVendorDetailsByUser, getPackageFacilityDetailsByPage]).subscribe(results => {
            ...
            if (data.length != 0) {
                ...
                let vendorIds = this.vendorDetails.map(x => x.vendorId);
                let Score = this.vendorDataService.getVendorData(this.pageNumber, this.sortColumn, this.sortDirection, this.pageSize, this.enterpriseId, PackageFacilityFilter, vendorIds.toString(), this.approveTransactionId);
                forkJoin([FacilityDetails, PackageTypeDetails, Score]).subscribe(results => {
                    let facility = results[0];
                    let packages = results[1];
                    let socore = results[2];
    ...
    roleWiseView(e) {
        ...
        this.isActiveRole = (role == "With Vendor") ? "With Vendor" : (role == "All" ? "All" : "");
        if (this.isActiveRole == "With Vendor") {
    ...
    public getFilteredData(): void {
        ...
        if (length == 0) {
            this.enableClearAll = false;
            this.vendorDataService.getVendorData(this.pageNumber, this.sortColumn, this.sortDirection, this.pageSize, this.enterpriseId, this.PackageFacilityFilter, vendorIds.toString(), this.approveTransactionId).subscribe(
                data => {
                    this.userData = data;
            ...
            this.vendorDataService.getVendorDataByFilter(this.selectedTransactionCode, this.selectedPackageTypeIds, selectedFacilityId, this.selectedVendorIds, this.selectedStatusIds
                , transactionDate, this.pageNumber, this.sortColumn, this.sortDirection, this.pageSize, this.enterpriseId, PackageFacilityFilter, vendorIds.toString(), MonthYearFilter, this.approveTransactionId).subscribe(
                    data => {
                        this.userData = data;
                        if (this.userData != null) {
                            this.count = this.userData[0][0].TotalRecordCount;
    ...
    approveScore() {
        ...
        if (this.comments == "" || this.comments == null) {
            ...
            this.confirm(configuration.Confirm, configuration.ConfirmAgree)
                .then((confirmed) => {
                        ...
                        this.scoreDataService.getTransactionStatus(this.transactionId).subscribe(
                            data => {
                                ...
                                if (data != currentStatusId) {
                                    ...
                                    modalRef.componentInstance.message = configuration.TransactionAgreedRevalidate;
                                    this.loadingSymbolForSubmitModal = false;
                                }
                                else {
                                    let currentDate = this.datepipe.transform(new Date(), 'yyyy-MM-dd HH:mm:ss');
                                    var statusId = this.statusDetails.find(x => x.statusName == configuration.AgreedandClosed).statusId;
                                    let saveRemarks = this.vendorDataService.saveRemarks(this.transactionId, this.comments, this.loggedEnterpriseId, currentDate, this.remarksBy);
                                    let updateTransaction = this.vendorDataService.updateTransactionStatus(this.transactionId, statusId);
                                    ...
                                    forkJoin([saveRemarks, updateTransaction, getCategoryNameByPackageId]).subscribe(results => {
                                        ...
                                        if (categoryName == configuration.CategoryME) {
                                            RoleIds.push(configuration.RoleZL, configuration.RoleMELead, configuration.RoleDE, configuration.RoleCE, configuration.RoleSRMAdmin);
                                        }
                                        else if (categoryName == configuration.CategorySoftServices) {
                                            RoleIds.push(configuration.RoleSDL, configuration.RoleWM, configuration.RoleDM, configuration.RoleCityLead, configuration.RoleSRMAdmin);
                                        }
                                        this.dataService.GetMailerDetails(this.facilityId, this.packageTypeId, RoleIds).subscribe(data => {
                                            let To = "";
                                            let Cc = "";
                                            if (categoryName == configuration.CategoryME) {
                                                for (var i = 0; i < data.length; i++) {
                                                    if (data[i]["RoleNm"] == configuration.RoleCE || data[i]["RoleNm"] == configuration.RoleDE)
                                                        To = To + data[i]["EnterpriseId"] + ";";
                                                    if (data[i]["RoleNm"] == configuration.RoleMELead || data[i]["RoleNm"] == configuration.RoleSRMAdmin
                                                        || data[i]["RoleNm"] == configuration.RoleZL)
                                                        Cc = Cc + data[i]["EnterpriseId"] + ";";
                                                }
                                            }
                                            else if (categoryName == configuration.CategorySoftServices) {
                                                for (var i = 0; i < data.length; i++) {
                                                    if (data[i]["RoleNm"] == configuration.RoleWM || data[i]["RoleNm"] == configuration.RoleDM)
                                                        To = To + data[i]["EnterpriseId"] + ";";
                                                    if (data[i]["RoleNm"] == configuration.RoleCityLead || data[i]["RoleNm"] == configuration.RoleSRMAdmin
                                                        || data[i]["RoleNm"] == configuration.RoleSDL)
                                                        Cc = Cc + data[i]["EnterpriseId"] + ";";
                                                }
                                            }
                                            To = Array.from(new Set(To.split(';'))).join(";").toString();
                                            Cc = Array.from(new Set(Cc.split(';'))).join(";").toString();
                                            var subject = transactionRefNo + " - Accepted";
                                            let body = "<table>"
                                                + "<tr>Dear Users,</tr><br><br>"
                                                + "<tr>We acknowledge and accept the scores</tr><br><br>"
                                                + "<br><br>"
                                                + "<tr>Regards,</tr><br>"
                                                + "<tr>Service Relationship Management Team</tr>"
                                                + "</table>";
                                            this.alertMailService.SendEmail(body, subject, To, Cc, "Vendoragrees").subscribe(
                                                data => {
                                                    debugger;
                                                    console.log(data);
                                        ...
                                        this.loadingSymbolForSubmitModal = false;
                                        this.getFilteredData();
                                        this.close(configuration.Success, configuration.TransactionAgreed)
                                            .then((confirmed) => {
    ...
    rejectScore() {
        ...
        if (this.comments == "" || this.comments == null) {
            ...
            this.confirm(configuration.Confirm, configuration.ConfirmDisagree)
                .then((confirmed) => {
                    console.log(confirmed);
                    if (confirmed) {
                        this.scoreDataService.getTransactionStatus(this.transactionId).subscribe(
                            data => {
                                ...
                                if (data != currentStatusId) {
                                    ...
                                    let currentDate = this.datepipe.transform(new Date(), 'yyyy-MM-dd HH:mm:ss');
                                    var statusId = this.statusDetails.find(x => x.statusName == configuration.VendorDisagreed).statusId;
                                    var date: Date = new Date(this.periodDate);
                                    var year: number = date.getFullYear();
                                    var month: number = date.getMonth();
                                    var lastDay: Date = new Date(year, month + 1, 0);
                                    ...
                                    if (lastDay.getDate() == date.getDate() && DailyWeeklYFortnightlyContains !== 0) {
                                        ...
                                        let saveRemarks = this.vendorDataService.saveRemarksBulk(this.packageTypeId, this.vendorId, this.facilityId, periodDate, this.comments, this.loggedEnterpriseId, currentDate, this.remarksBy);
                                        let updateTransaction = this.vendorDataService.updateTransactionStatusBulk(this.packageTypeId, this.vendorId, this.facilityId, periodDate, statusId);
                                        let getCategoryNameByPackageId = this.dataService.getCategoryNameByPackageId(this.packageTypeId);
                                        forkJoin([saveRemarks, updateTransaction, getCategoryNameByPackageId]).subscribe(results => {
                                            //const d = results[0];
                                            //this.loadingSymbolForSubmitModal = false;
                                            ...
                                            this.dataService.GetMailerDetails(this.facilityId, this.packageTypeId, RoleIds).subscribe(data => {
                                                ...
                                                let body = "<table>"
                                                    + "<tr>Dear Users,</tr><br><br>"
                                                    + "<tr>The vendor has disagreed to the score.</tr><br>"
                                                    + "<tr>Comments - " + comments +"</tr>"
                                                    + "<br><br>"
                                                    + "<tr>Regards,</tr><br>"
                                                    + "<tr>Service Relationship Management Team</tr>"
                                                    + "</table>";
                                            ...
                                            this.loadingSymbolForSubmitModal = false;
                                            this.getFilteredData();
                                            this.close(configuration.Success, configuration.TransactionDisagreed)
                                                .then((confirmed) => {
                                                    this.dismissSubmitScoreModal();
                                                })
                                                .catch(() => console.log('User dismissed the dialog (e.g., by using ESC, clicking the cross icon, or clicking outside the dialog)'));
    ...
    getViewScoreCard(firstDay: Date, lastDay: Date, date: Date, DailyWeeklYFortnightlyContains: number) {
        ...
        //this.vendorDataService.getRemarks(this.viewTransactionId).subscribe(
        this.vendorDataService.getRemarks(this.viewTransactionId, this.remarksBy).subscribe(
            data => {
                if (data.length != 0) {
    ...
    search = (text$: Observable<string>) =>
        text$.pipe(
            ...
            switchMap(term => term.trim().length < 3 ? [] :
                this.vendorDataService.searchTransaction(term.trim(), this.PackageFacilityAliasFilter, this.vendorDetails.map(x => x.vendorId).toString()).pipe(
